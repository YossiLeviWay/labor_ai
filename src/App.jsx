...
import { 
  ...
  Settings, Database, Upload, Heart, Edit, Trash2, LayoutDashboard, Bookmark, List, ShieldAlert, MapPin, ArrowRight, Bell, Clock, AlertCircle, Phone, Building, Users, RefreshCw
} from 'lucide-react';
...
const App = () => {
  ...
  const [formData, setFormData] = useState({
    name: '', contactPerson: '', contactPhone: '', providerType: 'ספק פרטי', 
    description: '', rating: 5, recommends: true, notes: '', grades: [], isZalash: false, location: 'כל הארץ'
  });

  const [filters, setFilters] = useState({ 
    search: '', grade: 'all', zalashOnly: false, location: 'all', minRating: 0, minReviewerRating: 0, providerType: 'all'
  });
  ...
  useEffect(() => {
    ...
    const unsubP = onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'programs'), (snap) => {
      const data = snap.docs.map(doc => {
        const p = doc.data();
        // Calculate average reviewer rating on the fly
        const reviewerRatings = p.comments?.map(c => c.rating).filter(r => r > 0) || [];
        const avgReviewerRating = reviewerRatings.length > 0 
          ? (reviewerRatings.reduce((a, b) => a + b, 0) / reviewerRatings.length).toFixed(1)
          : 0;
        
        return { id: doc.id, ...p, avgReviewerRating };
      });
      setPrograms(data);
      setLoading(false);
    });
    ...
    myProgs.forEach(p => {
      if (p.comments && p.comments.length > 0) {
        p.comments.slice(-3).forEach(c => {
          newNotifs.push({ title: `תגובה ודירוג חדש ל-${p.name}`, text: `${c.school} דירג ${c.rating} כוכבים: ${c.text}` });
        });
      }
    });
  ...
  const handleAddProgram = async (e) => {
    ...
    try {
      ...
      setView('dashboard');
      setFormData({ name: '', contactPerson: '', contactPhone: '', providerType: 'ספק פרטי', description: '', rating: 5, recommends: true, notes: '', grades: [], isZalash: false, location: 'כל הארץ' });
    } catch (err) { console.error(err); }
  };
  ...
  const addComment = async (programId, text, rating) => {
    const now = Date.now();
  ...
  const filteredPrograms = useMemo(() => {
    ...
    // Randomize order for the dashboard
    if (view === 'dashboard') {
       // Using randomSeed to reshuffle only when requested or on load
    ...
    return list.filter(p => {
      ...
      const matchesReviewerRating = p.avgReviewerRating >= filters.minReviewerRating;
      const matchesType = filters.providerType === 'all' || p.providerType === filters.providerType;

      return matchesSearch && matchesGrade && matchesZalash && matchesLocation && matchesRating && matchesReviewerRating && matchesType;
    });
  }, [programs, filters, user, view, randomSeed]);
        ...
        <form onSubmit={(e) => handleProviderApply(e, data)} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-right">
            <div className="col-span-1 md:col-span-2">
              <label className="block text-xs font-bold text-slate-400 mb-1 mr-1 uppercase">שם הספק / העמותה *</label>
            ...
            <div>
              <label className="block text-xs font-bold text-slate-400 mb-1 mr-1 uppercase">ח"פ / מזהה ייחודי (להתחברות) *</label>
            ...
            <div>
              <label className="block text-xs font-bold text-slate-400 mb-1 mr-1 uppercase">סיסמה מבוקשת *</label>
...
const ProviderStatusPage = ({ provider, setView }) => (
  <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4" dir="rtl">
    <div className="bg-white p-12 rounded-[3rem] shadow-2xl w-full max-w-md border border-slate-200 text-center relative overflow-hidden">
      ...
      <div className={`w-24 h-24 mx-auto rounded-full flex items-center justify-center mb-8 ${provider.status === 'pending' ? 'bg-amber-50 text-amber-600' : provider.status === 'approved' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>
        {provider.status === 'pending' ? <Clock size={48}/> : provider.status === 'approved' ? <CheckCircle2 size={48}/> : <AlertCircle size={48}/>}
      </div>
      <h2 className="text-2xl font-black text-slate-800 mb-2">סטטוס: {provider.status === 'pending' ? 'בקשתך בהמתנה' : provider.status === 'approved' ? 'אושרת כספק!' : 'בקשתך נדחתה'}</h2>
      <p className="text-slate-500 font-medium mb-8 text-right">שלום {provider.name}, בקשת הצטרפותך נמצאת בבדיקת המערכת.</p>
      {provider.adminNote && (
        <div className="bg-slate-50 p-6 rounded-2xl border mb-8 text-right shadow-inner">
          <p className="text-[10px] font-black text-slate-400 uppercase mb-2 text-right">הערת מנהל המערכת:</p>
          <p className="text-slate-800 text-sm font-medium italic text-right">"{provider.adminNote}"</p>
        </div>
      )}
      {provider.status === 'approved' && <button onClick={() => setView('dashboard')} className="w-full bg-blue-700 text-white font-bold py-5 rounded-2xl shadow-xl mb-4 text-lg">כניסה למאגר</button>}
      {provider.status === 'rejected' && <button onClick={() => setView('provider-register')} className="w-full bg-emerald-600 text-white font-bold py-5 rounded-2xl shadow-xl mb-4 text-lg">הגשת בקשה מעודכנת</button>}
        ...
        <div className="flex items-center gap-6">
          ...
          <nav className="flex gap-4 mr-6">
            <button onClick={() => setActiveTab('requests')} className={`px-4 py-2 rounded-xl text-sm font-bold relative transition-all ${activeTab === 'requests' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}>
              בקשות ספקים {pendingRequests.length > 0 && <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] w-5 h-5 flex items-center justify-center rounded-full border-2 border-slate-900 animate-bounce font-bold">{pendingRequests.length}</span>}
            </button>
      ...
      <main className="container mx-auto p-8 max-w-7xl">
        ...
        {activeTab === 'schools' && (
           <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 text-right">
              <div className="lg:col-span-1"><div className="bg-white p-6 rounded-[2rem] shadow-sm border"><h3 className="font-bold mb-4 flex items-center gap-2"><Upload size={18}/> ייבוא מאקסל</h3><textarea className="w-full h-40 p-4 bg-slate-50 rounded-xl mb-4 text-xs font-mono text-right" placeholder="שם מוסד	סמל	סיסמה" value={importText} onChange={e => setImportText(e.target.value)} /><button onClick={() => alert("ייבוא בביצוע...")} className="w-full bg-blue-600 text-white py-3 rounded-xl font-bold">ייבוא מוסדות</button></div></div>
              <div className="lg:col-span-2"><div className="bg-white rounded-[2rem] shadow-sm border overflow-hidden"><table className="w-full text-right"><thead className="bg-slate-50 text-[10px] uppercase font-black tracking-widest text-slate-400"><tr><th className="p-4">מוסד</th><th className="p-4">סמל</th><th className="p-4 text-center">פעולות</th></tr></thead><tbody className="divide-y text-sm">{schools.map(s => (<tr key={s.id} className="hover:bg-slate-50"><td className="p-4 font-bold">{s.name}</td><td className="p-4 font-mono">{s.semel}</td><td className="p-4 flex justify-center gap-2"><button onClick={() => setEditingSchool(s)} className="p-2 text-blue-600"><Edit size={16}/></button><button onClick={() => handleDeleteSchool(s.id)} className="p-2 text-red-500"><Trash2 size={16}/></button></td></tr>))}</tbody></table></div></div>
           </div>
        )}
      </main>
    </div>
...
const Dashboard = ({ setView, filters, setFilters, filteredPrograms, toggleFavorite, savedProgramIds, setSelectedProgram, user, handleDeleteProgram, notifications, setRandomSeed }) => (
  <div className="min-h-screen bg-slate-50 pb-20" dir="rtl">
    ...
    <main className="container mx-auto px-6 p-4 max-w-7xl">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10 mt-10 text-right">
        <div>
          ...
          <p className="text-slate-500 font-medium">המערכת מציגה תכניות באופן אקראי כדי לתת הזדמנות שווה לכולם.</p>
        </div>
        <div className="flex gap-4">
          <button onClick={() => setRandomSeed(Math.random())} className="flex items-center gap-2 bg-slate-200 text-slate-700 px-6 py-5 rounded-2xl font-black transition-all hover:bg-slate-300">
            <RefreshCw size={20} /> רענן תצוגה אקראית
          </button>
          {!user?.isProvider && (
      ...
      <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200 mb-12 space-y-6 text-right">
        <div className="flex flex-wrap gap-6 items-end">
          <div className="flex-1 min-w-[300px] relative">
            ...
            <input type="text" placeholder="חיפוש חופשי (שם, תוכן, ספק)..." className="w-full pr-14 pl-6 py-4 bg-slate-50 border-none rounded-2xl outline-none focus:ring-2 focus:ring-blue-500 text-right font-medium text-lg" value={filters.search} onChange={e => setFilters({...filters, search: e.target.value})} />
          </div>
          <div className="w-full sm:w-auto">
            <label className="text-[10px] font-black text-slate-400 uppercase block mb-2 mr-2">מיקום גיאוגרפי</label>
          ...
          <div className="w-full sm:w-auto">
            <label className="text-[10px] font-black text-slate-400 uppercase block mb-2 mr-2">סוג ספק</label>
            <select className="p-4 bg-slate-50 border-none rounded-2xl font-bold text-slate-700 min-w-[150px]" value={filters.providerType} onChange={e => setFilters({...filters, providerType: e.target.value})}>
              <option value="all">הכל</option>
              <option value="חברה">חברה</option><option value="ספק פרטי">ספק פרטי</option>
            </select>
          </div>
        </div>
        
        <div className="flex flex-wrap gap-8 pt-6 border-t border-slate-50 items-center justify-end">
          <div className="flex items-center gap-3">
            <span className="text-xs font-black text-slate-400 uppercase tracking-tighter">דירוג מוסדות (מינימלי):</span>
            <div className="flex gap-1.5">
              {[1,2,3,4,5].map(star => (
                <button key={star} onClick={() => setFilters({...filters, minRating: star})} className="transition-transform hover:scale-125">
                  <Star size={18} fill={star <= filters.minRating ? "#f59e0b" : "none"} className={star <= filters.minRating ? "text-amber-500" : "text-slate-200"} />
                </button>
              ))}
            </div>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-xs font-black text-slate-400 uppercase tracking-tighter">דירוג מגיבים (מינימלי):</span>
            <div className="flex gap-1.5">
              {[1,2,3,4,5].map(star => (
                <button key={star} onClick={() => setFilters({...filters, minReviewerRating: star})} className="transition-transform hover:scale-125">
                  <Star size={18} fill={star <= filters.minReviewerRating ? "#10b981" : "none"} className={star <= filters.minReviewerRating ? "text-emerald-500" : "text-slate-200"} />
                </button>
              ))}
            </div>
          </div>
          <label className="flex items-center gap-3 cursor-pointer group bg-slate-50 px-6 py-3 rounded-full"><input type="checkbox" className="w-5 h-5 rounded-md text-purple-600" checked={filters.zalashOnly} onChange={e => setFilters({...filters, zalashOnly: e.target.checked})} /><span className="text-sm font-black text-slate-700">מתאים לתכנית צל"ש</span></label>
        </div>
      </div>
        ...
        {filteredPrograms.map(p => (
          <div key={p.id} onClick={() => setSelectedProgram(p)} className="bg-white rounded-[2rem] shadow-sm border overflow-hidden hover:shadow-2xl transition-all group cursor-pointer relative text-right">
            <div className="absolute top-5 left-5 flex gap-2 z-10">
              {(!user?.isAdmin && !user?.isProvider) && (<button onClick={(e) => toggleFavorite(e, p.id)} className={`p-2.5 rounded-full transition-all ${savedProgramIds.includes(p.id) ? 'bg-rose-500 text-white' : 'bg-white/90 backdrop-blur shadow-sm text-slate-300 hover:text-rose-400'}`}><Heart size={18} fill={savedProgramIds.includes(p.id) ? "currentColor" : "none"} /></button>)}
              {user?.isAdmin && (<button onClick={(e) => { e.stopPropagation(); handleDeleteProgram(p.id); }} className="p-2.5 rounded-full bg-white/90 text-red-500 hover:bg-red-500 hover:text-white transition-all"><Trash2 size={18} /></button>)}
            </div>
            ...
            <div className="p-8">
              ...
              <div className="flex gap-4 mb-4">
                 <div className="flex text-amber-500 items-center gap-1"><Star size={14} fill="currentColor" /><span className="text-xs font-black">{p.rating}.0</span></div>
                 <div className="flex text-emerald-500 items-center gap-1"><Star size={14} fill="currentColor" /><span className="text-xs font-black">{p.avgReviewerRating > 0 ? `${p.avgReviewerRating} (מגיבים)` : 'טרם דורג'}</span></div>
              </div>
              <p className="text-slate-500 text-sm line-clamp-3 mb-6 leading-relaxed h-15">{p.description}</p>
              <div className="flex flex-wrap gap-2 mb-6">
                  ...
                  <span className="text-[10px] bg-blue-50 text-blue-700 px-4 py-1.5 rounded-full font-black uppercase flex items-center gap-1"><Building size={12} /> {p.providerType}</span>
              </div>
              <div className="flex justify-between items-center pt-6 border-t text-[10px] text-slate-400 font-black uppercase"><span>{p.schoolName}</span><span>{p.comments?.length || 0} תגובות</span></div>
            </div>
          </div>
        ))}
      </div>
...
const AddProgram = ({ setView, handleAddProgram, formData, setFormData, user }) => (
  <div className="min-h-screen bg-slate-50 pb-12" dir="rtl">
      <header className="bg-white border-b p-6 sticky top-0 z-30 flex items-center gap-4 shadow-sm text-right"><button onClick={() => setView('dashboard')} className="p-3 hover:bg-slate-100 rounded-full"><ChevronRight size={28} /></button><h1 className="text-2xl font-black text-slate-800">הוספת תכנית חדשה</h1></header>
      <main className="container mx-auto p-4 max-w-4xl mt-12 text-right">
          <form onSubmit={handleAddProgram} className="bg-white p-12 rounded-[3rem] shadow-2xl space-y-10 text-right">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="col-span-2"><label className="block text-sm font-black mb-3 text-slate-800">שם התכנית / הספק המבצע *</label><input required className="w-full p-5 bg-slate-50 border-none rounded-2xl outline-none focus:ring-2 focus:ring-blue-500 font-black text-xl text-right" placeholder="למשל: סדנת חוסן מנטלי" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} /></div>
                <div><label className="block text-sm font-black mb-3 text-slate-800">מיקום גיאוגרפי *</label><select required className="w-full p-5 bg-slate-50 border-none rounded-2xl font-black text-right" value={formData.location} onChange={e => setFormData({...formData, location: e.target.value})}><option value="כל הארץ">כל הארץ</option><option value="צפון">צפון</option><option value="דרום">דרום</option><option value="מרכז">מרכז</option></select></div>
                <div><label className="block text-sm font-black mb-3 text-slate-800">סוג ספק *</label><select required className="w-full p-5 bg-slate-50 border-none rounded-2xl font-black text-right" value={formData.providerType} onChange={e => setFormData({...formData, providerType: e.target.value})}><option value="חברה">חברה</option><option value="ספק פרטי">ספק פרטי</option></select></div>
                <div><label className="block text-sm font-black mb-3 text-slate-800">שם איש קשר</label><input className="w-full p-5 bg-slate-50 border-none rounded-2xl outline-none text-right" value={formData.contactPerson} onChange={e => setFormData({...formData, contactPerson: e.target.value})} /></div>
                <div><label className="block text-sm font-black mb-3 text-slate-800">מספר טלפון ליצירת קשר</label><input type="tel" className="w-full p-5 bg-slate-50 border-none rounded-2xl outline-none text-right" value={formData.contactPhone} onChange={e => setFormData({...formData, contactPhone: e.target.value})} /></div>
                <div className="col-span-2">
                   <label className="block text-sm font-black mb-3 text-slate-800">דירוג המוסד המדווח (1-5)</label>
                   <div className="flex gap-4 p-4 bg-slate-50 rounded-2xl justify-center">{[1,2,3,4,5].map(s => (<button key={s} type="button" onClick={() => setFormData({...formData, rating: s})}><Star size={32} fill={s <= formData.rating ? "#f59e0b" : "none"} className={s <= formData.rating ? "text-amber-500" : "text-slate-200"} /></button>))}</div>
                </div>
              </div>
              <div><label className="block text-sm font-black mb-3 text-slate-800">מהות ותכני התכנית (פירוט) *</label><textarea required rows="5" className="w-full p-6 bg-slate-50 border-none rounded-3xl outline-none text-right font-medium" placeholder="פרט כאן על התכנים..." value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} /></div>
...
const ProgramDetails = ({ selectedProgram, setView, addComment, user, removeComment, editComment, notifications }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-right" dir="rtl">
        <Header setView={setView} user={user} notifications={notifications} />
        <main className="container mx-auto p-6 max-w-6xl mt-12 text-right">
            <div className="bg-white p-12 md:p-16 rounded-[3.5rem] shadow-2xl space-y-12 border relative overflow-hidden text-right">
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start gap-12">
                  <div className="flex-1 space-y-8">
                    <div className="flex flex-wrap items-center gap-4 justify-start">
                       ...
                       <span className="text-sm font-black text-blue-600 flex items-center gap-2 bg-blue-50 px-4 py-2 rounded-full"><Building size={16}/> {selectedProgram.providerType}</span>
                    </div>
                    ...
                    {(selectedProgram.contactPerson || selectedProgram.contactPhone) && (
                      <div className="bg-blue-50 p-6 rounded-3xl border border-blue-100 mt-6">
                         <h4 className="font-black text-blue-900 mb-2">פרטי התקשרות:</h4>
                         <div className="flex gap-6">
                            {selectedProgram.contactPerson && <div className="flex items-center gap-2 text-slate-700 font-bold"><User size={18}/> {selectedProgram.contactPerson}</div>}
                            {selectedProgram.contactPhone && <div className="flex items-center gap-2 text-slate-700 font-bold"><Phone size={18}/> {selectedProgram.contactPhone}</div>}
                         </div>
                      </div>
                    )}
                  </div>
                  <div className="bg-slate-50 p-10 rounded-[2.5rem] w-full lg:w-80 border shadow-inner">
                    <div className="space-y-6">
                      <div className="flex items-center justify-between"><span className="text-xs font-bold text-slate-400">דירוג מוסד:</span><div className="flex items-center gap-2"><span className="font-black text-2xl">{selectedProgram.rating}.0</span><Star className="text-amber-500" fill="currentColor" size={24}/></div></div>
                      <div className="flex items-center justify-between"><span className="text-xs font-bold text-slate-400">דירוג מגיבים:</span><div className="flex items-center gap-2"><span className="font-black text-2xl text-emerald-600">{selectedProgram.avgReviewerRating || '0.0'}</span><Star className="text-emerald-500" fill="currentColor" size={24}/></div></div>
                    </div>
                    <div className="pt-8 border-t mt-6 text-right"><p className="text-[10px] font-black text-slate-400 uppercase">דווח על ידי</p><p className="font-black text-blue-900 text-xl">{selectedProgram.schoolName}</p></div>
                  </div>
                </div>
                <div className="border-t pt-16 text-right">
                    <h3 className="text-3xl font-black mb-10 flex items-center gap-4 text-slate-800 justify-start"><MessageSquare className="text-blue-600" size={32}/> תגובות וחוות דעת ({selectedProgram.comments?.length || 0})</h3>
                    <div className="space-y-6 mb-16">
                        {selectedProgram.comments?.map((c) => (
                            <div key={c.id} className="bg-slate-50 p-8 rounded-[2rem] border-r-8 border-blue-600 flex justify-between items-start group">
                                <div className="flex-1 ml-6 text-right">
                                  <div className="flex items-center gap-4 mb-2">
                                    <div className="font-black text-blue-900 text-sm">{c.school} {c.edited && <span className="text-[10px] text-slate-400 mr-2 font-normal">(נערך)</span>}</div>
                                    <div className="flex text-emerald-500"><Star size={14} fill="currentColor" /> <span className="text-xs font-black mr-1">{c.rating || 5}</span></div>
                                  </div>
                                  {editingId === c.id ? (
                                    <div className="flex flex-col gap-3"><textarea className="w-full p-4 border rounded-xl text-right" value={editText} onChange={e => setEditText(e.target.value)} /><div className="flex gap-2"><button onClick={saveEdit} className="bg-blue-600 text-white px-4 py-1 rounded-lg text-sm font-bold">שמור</button><button onClick={() => setEditingId(null)} className="bg-slate-200 px-4 py-1 rounded-lg text-sm font-bold">ביטול</button></div></div>
                                  ) : ( <p className="text-slate-700 text-lg font-medium">{c.text}</p> )}
                                </div>
                                <div className="flex gap-2">
                                  {c.schoolId === user.id && editingId !== c.id && ( <button onClick={() => handleEdit(c)} className="p-2 text-blue-400 hover:text-blue-600 opacity-0 group-hover:opacity-100 transition-opacity"><Edit size={18}/></button> )}
                                  {(user?.isAdmin || c.schoolId === user.id) && ( <button onClick={() => removeComment(selectedProgram.id, c.id)} className="p-3 text-red-500 hover:bg-red-50 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={20} /></button> )}
                                </div>
                            </div>
                        ))}
                    </div>
                    {!user?.isProvider && (
                      <div className="bg-blue-50/50 p-10 rounded-[3rem] border shadow-inner">
                          <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                             <label className="block text-sm font-black text-blue-900 tracking-wide uppercase">הוסף חוות דעת ודירוג משלך:</label>
                             <div className="flex items-center gap-3 bg-white px-6 py-2 rounded-2xl shadow-sm border border-blue-100">
                                <span className="text-xs font-bold text-slate-400">דירוג שלך:</span>
                                <div className="flex gap-1">
                                   {[1,2,3,4,5].map(star => (
                                      <button key={star} onClick={() => setNewRating(star)} className="transition-transform hover:scale-125">
                                         <Star size={24} fill={star <= newRating ? "#10b981" : "none"} className={star <= newRating ? "text-emerald-500" : "text-slate-200"} />
                                      </button>
                                   ))}
                                </div>
                             </div>
                          </div>
                          <textarea id="newComment" className="w-full p-6 bg-white border rounded-3xl outline-none text-right font-medium text-lg h-32 mb-6" placeholder="איך הייתה החוויה שלכם מהתכנית?" />
                          <button onClick={() => { const v = document.getElementById('newComment').value; if(v) { addComment(selectedProgram.id, v, newRating); document.getElementById('newComment').value=''; } }} className="bg-blue-700 hover:bg-blue-800 text-white px-12 py-5 rounded-2xl font-black shadow-2xl text-lg">פרסם תגובה ודירוג</button>
                      </div>
                    )}
                </div>
            </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-12" dir="rtl">
      ...
      <main className="container mx-auto p-8 max-w-6xl mt-10 text-right">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          <div className="bg-white rounded-[2.5rem] shadow-sm border p-8"><h3 className="text-2xl font-black mb-8 flex items-center gap-3 text-blue-800"><List size={24}/> תכניות שלי ({myUploads.length})</h3>{myUploads.map(p => (<div key={p.id} className="flex justify-between items-center p-6 bg-slate-50 rounded-3xl mb-4 cursor-pointer" onClick={() => setSelectedProgram(p)}><div className="text-right"><h4 className="font-black text-lg">{p.name}</h4><p className="text-xs text-slate-400">{p.createdAt}</p></div><ChevronRight className="text-blue-600"/></div>))}</div>
          <div className="bg-white rounded-[2.5rem] shadow-sm border p-8"><h3 className="text-2xl font-black mb-8 flex items-center gap-3 text-rose-600"><Bookmark size={24}/> תכניות במעקב ({savedPrograms.length})</h3>{savedPrograms.map(p => (<div key={p.id} className="flex justify-between items-center p-6 bg-rose-50/30 rounded-3xl mb-4 cursor-pointer" onClick={() => setSelectedProgram(p)}><div className="text-right"><h4 className="font-black text-lg">{p.name}</h4><p className="text-[10px] text-slate-400">מאת: {p.schoolName}</p></div><button onClick={(e) => { e.stopPropagation(); toggleFavorite(e, p.id); }} className="p-2 text-rose-600"><Heart fill="currentColor"/></button></div>))}</div>
        </div>
      </main>
    </div>
  );
